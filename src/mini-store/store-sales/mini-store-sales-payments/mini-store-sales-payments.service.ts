...
import {
  QueryBilling,
  QuerySimpleReport,
} from './interface/InvoiceMiniStore.interface';
...
@Injectable()
export class MiniStoreSalesPaymentsService extends TypeOrmCrudService<
...
> {
  constructor(
    ...
    readonly salesRepository: Repository<MiniStoreSale>,
    ...
    @InjectRepository(SalesReturns, ColegioDBNameConnection)
  ...
  async countTotalPayments(dateStart: string, dateEnd: string, id: number) {
    return await this.repo
      ...
      .andWhere(
        `DATE(payments.createdAt) BETWEEN '${dateStart}' AND '${dateEnd}'`,
      )
      .getRawOne();
  }
  ...
  async fetchFilteredPayments(
    ...
    if (query) {
      ...
      if (query.cashier) {
        paymentsQueryBuilder.andWhere('agent.id = :agentID', {
  ...
  async fetchFilteredSales(query: QuerySimpleReport): Promise<MiniStoreSale[]> {
    ...
    if (query) {
      ...
      salesQueryBuilder.andWhere(
        'payments.createdAt BETWEEN :startDate AND :endDate',
        {
          startDate: moment(query.startDate).startOf('day').toDate(),
          endDate: moment(query.endDate).endOf('day').toDate(),
        },
      ...
      if (query.cashier) {
        salesQueryBuilder.andWhere('agent.id = :agentID', {
          agentID: query.cashier,
        });
      }
    }
  ...
  async findSaleByPayment(
    ...
    if (query.salePaymentId != 0) {
      ...
      sale.miniStoreSaleDetails.forEach((d: MiniStoreSaleDetail) => {
        ...
        quantitySum = Decimal.add(
          quantitySum,
          Decimal.mul(price, d.quantity),
        ).toNumber();
      });
    ...
    return {
      sale,
      payment,
      highestPayment: this.getHighestPayment(
        payment.miniStoreSaleMethodPayments,
      ),
    };
  }
  ...
  async fetchFilteredReturns(query: QuerySimpleReport) {
    ...
    if (query) {
      ...
      salesReturnsQB.andWhere(
        'saleReturn.createdAt BETWEEN :startDate AND :endDate',
        {
          startDate: moment(query.startDate).startOf('day').toDate(),
          endDate: moment(query.endDate).endOf('day').toDate(),
        },
      ...
      if (query.cashier) {
        salesReturnsQB.andWhere('agent.id = :agentID', {
          agentID: query.cashier,
        });
      }
    }
