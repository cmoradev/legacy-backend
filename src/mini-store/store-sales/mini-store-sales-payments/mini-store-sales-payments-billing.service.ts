...
@Injectable()
export class MiniStoreSalesPaymentsBillingService extends TypeOrmCrudService<MiniStoreSalePayment> {
    constructor(
        @InjectRepository(MiniStoreSalePayment, ColegioDBNameConnection)
        readonly repo: Repository<MiniStoreSalePayment>,
        @InjectRepository(MiniStoreInvoice, ColegioDBNameConnection)
        readonly invoiceRepository: Repository<MiniStoreInvoice>,
        @InjectRepository(MiniStoreSale, ColegioDBNameConnection)
        readonly miniStoreSaleRepo: Repository<MiniStoreSale>,
        ...
        @Inject(forwardRef(() => MiniStoreInvoicesService))
        private readonly miniStoreInvoicesService: MiniStoreInvoicesService,
        ...
        private readonly service: MiniStoreSalesPaymentsService,
    ) {
        super(repo);
    }
     ...
     */
    async checkExistingStamping(salePaymentId: number): Promise<{
        action: 'return_existing' | 'check_pending' | 'proceed';
        invoice?: MiniStoreInvoice;
        response?: BillingResponse;
    }> {
        // Caso 1: Ya existe factura con stamping=1
        const existingInvoiced = await this.miniStoreInvoicesService.findInvoiceByPayment({
        ...
        const pendingInvoice = await this.invoiceRepository
            ...
            .innerJoin('inv.miniStoreSalePayment', 'pay')
            .where('pay.id = :paymentId', {paymentId: salePaymentId})
        ...
        if (pendingInvoice) {
            ...
            try {
                ...
                await this.miniStoreInvoicesService.updateInvoice(pendingInvoice);
                ...
                return {
                    ...
                    response: {
                        stamping: true,
                        msg: 'PAGO_FACTURADO (recuperado de timbrado pendiente)',
                        invoice: pendingInvoice,
     ...
     */
    async processBilling(query: QueryBilling): Promise<BillingResponse> {
        return BillingLock.acquire(
            `store:${query.salePaymentId}`,
            () => this.processBillingLocked(query),
        );
    }

    private async processBillingLocked(query: QueryBilling): Promise<BillingResponse> {
        ...
        const existingCheck = await this.checkExistingStamping(query.salePaymentId);
        if (existingCheck.action === 'return_existing') {
            return existingCheck.response;
        }

        // ── 3. Buscar o crear registro de factura ──
        let invoiceRecord = await this.miniStoreInvoicesService.findInvoiceByPayment({
        ...
        if (!invoiceRecord) {
            ...
            factura.status = 0;
            factura.miniStoreSale = {id: query.saleId} as MiniStoreSale;
            factura.miniStoreSalePayment = {id: query.salePaymentId} as MiniStoreSalePayment;
            ...
            invoiceRecord = await this.miniStoreInvoicesService.saveInvoice(factura);
        }
        ...
        try {
            ...
            const updatedInvoice = await this.miniStoreInvoicesService.updateInvoice(invoiceRecord);
    ...
    /**
     * Orquestador principal del proceso de facturación global.
     */
    async processGlobalBilling(query: NotInvoicedDto): Promise<GlobalBillingResponse> {
        ...
        // ── 1. Obtener conceptos no facturados ──
        ...
        const fullResult: FullGenerateResult = await GenerateGlobalInvoiceMunyaal({
            branchOfficeConfig,
            wayPayment,
            details,
            env,
        ...
        // ── 3. Actualizar estados en BD (CRÍTICO con recuperación) ──
        try {
            await this.updateStampingPayments(concepts.map((value: NotInvoiced) => value.p_id), fullResult.uuid);
        ...
        try {
            await this.service.sendMail(branchOffice, fullResult.uuid, branchOfficeConfig.email);
            response.emailSent = true;
        } catch (err) {
            response.warnings.push({
                step: 'email',
        ...
        response.uuid = fullResult.uuid;
        response.stamping = fullResult.stamped;
