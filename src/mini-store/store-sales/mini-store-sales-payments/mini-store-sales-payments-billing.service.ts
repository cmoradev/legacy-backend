...
@Injectable()
export class MiniStoreSalesPaymentsBillingService extends TypeOrmCrudService<
...
> {
      ...
      try {
            ...
            warnings: [
              {
                step: 'recovery',
                message:
                  'Se recuperó un timbrado que no se había actualizado en BD',
              },
            ],
            statusUpdated: true,
            emailSent: false,
        ...
        return {
          ...
          response: {
            stamping: true,
            msg:
              'FACTURA TIMBRADA PERO CON ERROR EN ACTUALIZACIÓN DE BD. Contacte a soporte.',
            invoice: pendingInvoice,
  ...
  async processBilling(query: QueryBilling): Promise<BillingResponse> {
    return BillingLock.acquire(`store:${query.salePaymentId}`, () =>
      this.processBillingLocked(query),
    );
  }

  private async processBillingLocked(
    query: QueryBilling,
    ...
    try {
      ...
      const updatedInvoice = await this.miniStoreInvoicesService.updateInvoice(
        invoiceRecord,
      );
   ...
   */
  async processGlobalBilling(
    query: NotInvoicedDto,
  ): Promise<GlobalBillingResponse> {
