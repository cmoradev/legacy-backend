...
@Controller()
export class MiniStoreSalesPaymentsController
  implements CrudController<MiniStoreSalePayment> {
  constructor(
  ...
  @Post('/billing/resend-invoice')
  @UsePipes(ValidationPipe)
  public async resendInvoice(
    @Body() query: ResendInvoiceDto,
    @Res() res,
  ): Promise<any> {
    try {
      ...
      if (invoice.status !== InvoiceStatus.billed) {
        throw new BadRequestException('Invoice is not stamped');
      }
    ...
    try {
      ...
      if (result) {
        ...
        result.forEach((p, index) => {
          miniStoreSaleDetails.push(
            ...p.miniStoreSale.miniStoreSaleDetails.map((s) => {
              return {
                ...s,
                miniStoreSale: {
