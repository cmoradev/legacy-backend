...
export class MiniStoreSalesReturnsController implements CrudController<SalesReturns> {
    ...
    async verifyregistration(@Body() dataToInvoice: InvoiceSaleReturnDto, @Res() res: Response): Promise<any> {
        ...
        cfdi.emisor({
            Rfc: company.rfc, // 'TCM970625MB1',
            Nombre: company.businessName,
            RegimenFiscal: company.fiscalRegime, // '601',
        });
        cfdi.receptor({
        ...
        try {
            ...
            const data: { uuid, total } = await axios.post('https://api.signati.io/timbrado/facturar', cfdiObj)
                .then((res: any) => {
                    return res.data;
                });
