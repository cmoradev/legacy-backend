...
import {
  ...
  totalForCategory,
  totalForProducts,
} from './reports/mini-store-sale.report';
...
export class MiniStoreSalesController implements CrudController<MiniStoreSale> {
  ...
  async saleReport(
    ...
    if (query.type === 1 || query.type.toString() === '1') {
      result.products = totalForProducts(
        await this.service.reportCatCasherProd(query),
      );
    }
    if (query.type === 2 || query.type.toString() === '2') {
      result.category = totalForCategory(
        await this.service.reportCatCasherProd(query),
      );
    }
    if (query.type === 3 || query.type.toString() === '3') {
      result.cashier = totalForCashier(
        await this.paymentService.fetchFilteredPayments({
          status: query.status,
  ...
  private async reportSaleToday(
    ...
    data = result.map((d: any) => {
      ...
      return {
        ...d,
        id_estado_pago: parseInt(`${d.id_estado_pago}`),
        idsPagos: idsPagos.map((p: string) => {
          return parseInt(`${p}`);
        }),
        idsDetalles: idsDetalles.map((p: string) => {
          return parseInt(`${p}`);
        }),
      } as IReportSaleTodayRow;
    });
    ...
    if (options?.isExported) {
      const conceptStatusExcel = new SaleTodayExcel(
        options,
      ...
      const buffer = await conceptStatusExcel.getWorkBook().xlsx.writeBuffer({
        filename: `${
          getNameReport(
            options.byClient ? 'Ventas por cliente' : 'Ventas',
            options,
          ).excel
        }.xlsx`,
      });
  ...
  private async reportInformative(
    ...
    try {
        ...
        switch (parseInt(`${options.type}`)) {
          case TypeInformativeReport.PRODUCTS:
            const index = data.findIndex(
              (d: IReportInformativeRow) => d.p_id_product == r.p_id_product,
            );
            if (index > -1) {
              data[index].vd_quantity = Decimal.sum(
                data[index].vd_quantity,
                r.vd_quantity,
              ).toNumber();
              data[index].subtotal = Decimal.mul(
                data[index].vd_quantity,
                r.vd_price,
              ).toNumber();
            } else {
          ...
          case TypeInformativeReport.CATEGORIES:
            const indexCategory = data.findIndex(
              (d: IReportInformativeRow) => d.c_id == r.c_id,
            );
            if (indexCategory > -1) {
              data[indexCategory].vd_quantity = Decimal.sum(
                data[indexCategory].vd_quantity,
                r.vd_quantity,
              ).toNumber();
              const priceCategories = Decimal.mul(
                r.vd_quantity,
                r.vd_price,
              ).toNumber();
              ...
              data.push({
                ...r,
                subtotal: Decimal.mul(r.vd_quantity, r.vd_price).toNumber(),
              });
            }
            break;
          case TypeInformativeReport.CASHIERS:
            const indexCashier = data.findIndex(
              (d: IReportInformativeRow) => d.u_id_agent == r.u_id_agent,
            );
            if (indexCashier > -1) {
              data[indexCashier].vd_quantity = Decimal.sum(
                data[indexCashier].vd_quantity,
                r.vd_quantity,
              ).toNumber();
              const priceCashier = Decimal.mul(
                r.vd_quantity,
                r.vd_price,
              ).toNumber();
      ...
      if (options?.isExported) {
        const conceptStatusExcel = new InformativeExcel(
          options,
          data,
          dataFolios.sort((a, b) => a.p_id_product - b.p_id_product),
        );
        const buffer = await conceptStatusExcel.getWorkBook().xlsx.writeBuffer({
  ...
  private async reportSalesReturns(
    ...
    if (options?.isExported) {
      const conceptStatusExcel = new SaleReturnExcel(
        options,
        options.byClient ? dataByClient : data,
      );
      const buffer = await conceptStatusExcel.getWorkBook().xlsx.writeBuffer({
        filename: `${
          getNameReport(
            options.byClient ? 'Devoluciones_por_cliente' : 'Devoluciones',
            options,
          ).excel
        }.xlsx`,
      });
