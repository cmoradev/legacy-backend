...
export class MiniStoreInvoiceService {
  ...
  public async buildDocument(
    rows: MiniStoreInvoiceDetailsRow[],
    summary: MiniStoreIncomeSummaryRow[],
  ) {
   ...
   * @returns {Promise<MiniStoreInvoiceIncomeRow[]>} - Una promesa que resuelve a un array de detalles de ingresos de facturas.
   *
  ...
  private async getIncomes(
    paymentIDs: number[],
    UUIDs: string[],
  ): Promise<MiniStoreInvoiceIncomeRow[]> {
  ...
  private matchInvoicesToIncome(
    incomes: MiniStoreInvoiceIncomeRow[],
   ...
   */
  private getFiscalMethodPayment(incomes: MiniStoreInvoiceIncomeRow[]) {
   ...
   *
   * @param {MiniStoreInvoiceDetailsRow[]} rows - Lista de filas de detalles de facturas.
   ...
   */
  private buildSummary(rows: MiniStoreInvoiceDetailsRow[]) {
    ...
    const summary: MiniStoreIncomeSummaryRow[] = [];
