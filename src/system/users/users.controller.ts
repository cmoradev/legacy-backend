...
export class UsersController implements CrudController<User> {
  constructor(
    ...
    @InjectRepository(Teacher, ColegioDBNameConnection)
    readonly repoTeacher: Repository<Teacher>,
  ) {}
  ...
  @Get('/amir/data')
  async getuser() {
    const user: User = await this.service.repo
  ...
  async updateUserOfTeacher(
    @Param('idUser') idUser: string,
    @Body('teacher') teacherPayload: Partial<Teacher>,
    @Res() res: Response,
  ) {
    try {
      if (idUser.length < 1) {
      ...
      const user = await this.service.repo.findOneOrFail(idUser, {
        relations: ['teacher'],
      });
      ...
      const newTeacherPromiseFind = this.repoTeacher.findOneOrFail(
        teacherPayload.id,
        { relations: ['user'] },
      );
      const [newUser, newTeacher] = await Promise.all([
        newUserPromiseFind,
        newTeacherPromiseFind,
      ]);
    ...
    try {
      ...
      if (user) {
        ...
        const result = await this.service.save(
          await this.service.create(userBody),
        );
        res.send(result);
      }
    } catch (e) {
  ...
  async updatePassword(
    @Body() userBody: UpdatePasswordDto,
    @Req() req,
    @Res() res: Response,
  ) {
    try {
      ...
      const access = await compareSync(
        passwordOld,
      ...
      if (user) {
        if (access || isAdmin) {
          const result = await this.service.save(
            await this.service.changePassword(userBody),
          );
