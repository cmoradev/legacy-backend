...
import {
  ConceptReceipt,
  ConceptSchoolAndAcademy,
  DataInvoice,
  dataInvoiceInit,
} from './TypesCalculation';
...
export const ConceptsPriceByPaymentBilligCalculation = <
  T extends Detalles
>(payload: {
  payment: Payment;
  details: T[];
  ...
  const { payment, details, type, typeConcept, ivaByDetail = 0.16 } = payload;
  ...
  totals.detailsWithPaymentApplied.concepts.forEach((concept: Concept) => {
    ...
    if (typeConcept === 'Recepit') {
        ...
        cpt = {
          ...
          beca:
            scholarships.length > 0
              ? Decimal.sum(...scholarships).toFixed(2)
              : '0.00',
        } as ConceptSchoolAndAcademy;
      }
    } else {
      cpt = {
        concept: {
      ...
      if (
        ivaByDetail !== 0 &&
        conceptDetails.objetoImp === ObjetoImpEnum.OI02
      ) {
  ...
  obj.totals.fiscal.SubTotal = totals.detailsWithPaymentApplied.amount.toFixed(
    2,
  );
  obj.totals.fiscal.Descuento = totals.detailsWithPaymentApplied.discount.toFixed(
    2,
  );
  ...
  obj.totals.receipt = {
    ...
    Recargo:
      surchargesTotal.length > 0
        ? Decimal.sum(...surchargesTotal).toFixed(2)
        : '0.000',
...
const getPrice = <T extends Detalles>(
  detail: T,
  type: InvoiceModules,
): number => {
  if (type === InvoiceModules.STORE) {
...
export const getChargeDetails = <T extends Detalles>(
  details: T,
  type: InvoiceModules,
): Charge[] => {
