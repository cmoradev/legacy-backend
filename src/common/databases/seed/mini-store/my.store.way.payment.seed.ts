...
export default class MyStoreWayPaymentSeed implements Seeder {
    public async run(factory: Factory, connection: Connection): Promise<any> {
        await connection.query(`DROP VIEW IF EXISTS vw_my_tie_way_payments;`);
        await connection.query(`
        CREATE VIEW vw_my_tie_way_payments AS
        SELECT 
            p.id AS p_id,
            p.folio AS p_folio,
