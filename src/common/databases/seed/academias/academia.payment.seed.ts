...
export default class AcademiaPaymentSeed implements Seeder {
    public async run(factory: Factory, connection: Connection): Promise<any> {
        ...
        (select p_way_name from vw_sch_way_payments where p_id = v.id) AS p_metodo_pago,
        (select p_way from vw_sch_way_payments where p_id = v.id) AS p_metodo_pago_codigo,
