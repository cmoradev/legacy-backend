...
export const saleDetailsCalculations = (
    ...
    if (details.length !== 0) {
        return ({
            ...
            taxes: parseFloat(obj.impuestos.translados.Importe ? obj.impuestos.translados.Importe.toString() : '0.00'),
            data: obj.detalles,
        ...
        totals.concepts.forEach((c: Concept) => {
            c.charges.forEach((cc: Charge) => {
                if (cc.id == SystemTypeExtraChargesEnum.Recargos) {
                    surchargesTotal.push(c.chargeWithIVA ? c.chargeWithIVA : new Decimal(0.00))
                }
            });
        });

        resultad.detalles = totals.concepts ? totals.concepts : [];
        resultad.discount = totals.discount ? parseFloat(totals.discount.toFixed(2)) : 0.00,
            resultad.impuestos = {
                translados: {
                    Base: totals.baseTax ? parseFloat(totals.baseTax.toFixed(2)) : 0.00,
                    Importe: totals.tax ? parseFloat(totals.tax.toFixed(2)) : 0.00,
                }
            };
        resultad.subtotal = totals.amount ? parseFloat(totals.amount.toFixed(2)) : 0.00;
        resultad.surcharges = surchargesTotal.length > 0 ? parseFloat(Decimal.sum(...surchargesTotal).toFixed(2)) : 0.00,
            resultad.total = totals.total ? parseFloat(totals.total.toFixed(2)) : 0.00;
    }
    return resultad;
}
...
export const reportPaymentByClient = (
    data: any[]
) => {
    const dataClient = [];
    data.forEach((d: any) => {
...
export const dataFullSale = (value: any[], type: InvoiceModules) => {
    const data = getDataCharges(value, type, true).map((d) => {
        const totalsPayments = new Decimal(d.p_total_without_current != null ? d.p_total_without_current : 0).toNumber();
    ...
    data.forEach((d) => {
        ...
        d.sale_details.forEach((sd) => {
            const invoiceDetails = ConceptsPriceByPaymentBilligCalculation({
                payment: {
                    change: 0, quantity: saleDetailsCalculations({
                        details: [sd],
                        type
                    }).total
                },
            ...
            result.push({
                ...
                p_income: Decimal.sum(
                    ...
                    Decimal.sub(invoiceDetails.totals.receipt.SubTotal, invoiceDetails.totals.fiscal.Descuento)
                ).toNumber(),
