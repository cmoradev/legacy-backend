...
export const getDataMatrizPayments = (
  data: VWPaymentExtraCharge[],
  type: InvoiceModules,
    ...
    if (index == -1) {
      ...
      if (isInvoice) {
        ...
        iCashier = dataMatriz.cashiers.findIndex(
          (c) =>
            c.id ==
            (status == 1
              ? parseInt(`${d.cashier_id}`)
              : parseInt(`${d.cancelation_id}`)),
        );
      } else {
        // estatus 2 - activo
        iCashier = dataMatriz.cashiers.findIndex(
          (c) =>
            c.id ==
            (status == 2
              ? parseInt(`${d.p_cashier_id}`)
              : parseInt(`${d.p_cancelation_id}`)),
        );
      }
      imethodsPayments = dataMatriz.methodsPayments.findIndex(
        (m) =>
          m.id ==
          (isInvoice
            ? parseInt(
                d.f_metodo_pago_codigo != null
                  ? d.f_metodo_pago_codigo
                  : d.p_metodo_pago_codigo,
              )
      ...
      if (iCashier == -1) {
        ...
        if (isInvoice) {
          ...
          status == 1
            ? (objCashier = {
                id: parseInt(`${d.cashier_id}`),
                name: d.u_fullname_cashier,
              })
            : (objCashier = {
                id: parseInt(`${d.cancelation_id}`),
                name: d.us_fullname_cancelation,
              });
        } else {
          // estatus 2 - activo
          status == 2
            ? (objCashier = {
                id: parseInt(`${d.p_cashier_id}`),
                name: d.p_fullname_cashier,
              })
            : (objCashier = {
                id: parseInt(`${d.p_cancelation_id}`),
                name: d.p_fullname_cancelation,
              });
        }
        ...
        dataMatriz.methodsPayments.push(
          isInvoice
            ? ({
                ...
                name:
                  d.f_metodo_pago != null ? d.f_metodo_pago : d.p_metodo_pago,
              } as InvoiceMethodPayment)
            : ({
                id: parseInt(d.p_metodo_pago_codigo),
                name: d.p_metodo_pago,
              } as InvoiceMethodPayment),
        );
      }
            ...
            methodsPayments: [
              {
                ...
                invoiceMethodPayment: invoiceMethodPaymentObj as Partial<
                  InvoiceMethodPayment
                >,
              } as Partial<SchoolChargesMethodsPayments>,
            ] as SchoolChargesMethodsPayments[],
            ...
            methodsPayments: [
              {
                ...
                invoiceMethodPayment: invoiceMethodPaymentObj as Partial<
                  InvoiceMethodPayment
                >,
              } as Partial<AcademyChargeMethodsPayments>,
            ] as AcademyChargeMethodsPayments[],
...
export const getMatrizPayments = (
  payments:
    | SchoolChargePayment[]
    | MiniStoreSalePayment[]
    | AcademyChargePayments[],
  ...
  const headers: any[] = [
    'Tipo',
    ...cashiers.map((value: User) => value && value.name),
    'Total',
  ];
  ...
  methodsPayments.forEach((paymentMethod) => {
    ...
    switch (type) {
      case InvoiceModules.SCHOOL:
        ...
        paymentsByMethod = paymentSchool.filter((payment) =>
          payment.methodsPayments.some(
            (method) => method.invoiceMethodPayment.id === paymentMethod.id,
          ),
        );
        break;
      case InvoiceModules.STORE:
        const paymentStore = payments as MiniStoreSalePayment[];
        paymentsByMethod = paymentStore.filter((payment) =>
          payment.miniStoreSaleMethodPayments.some(
            (method) => method.invoiceMethodPayment.id === paymentMethod.id,
          ),
        );
        break;
      case InvoiceModules.ACADEMY:
        ...
        paymentsByMethod = paymentAcademy.filter((payment) =>
          payment.methodsPayments.some(
            (method) => method.invoiceMethodPayment.id === paymentMethod.id,
          ),
        );
  ...
  for (const paymentMethod of methodsPayments) {
    ...
    for (const cashier of cashiers) {
      const filteredResume = resume.filter(
        (value) =>
          value.paymentMethod.id === paymentMethod.id &&
          value.cashier.id === cashier.id,
      );
      resumeDataTableItem.push(
        filteredResume.reduce((previousValue, currentValue) => {
...
export const getDataFullMatrizAndData = (
  result: VWPaymentExtraCharge[] = [],
  type: InvoiceModules,
  isInvoice: boolean = false,
  status: number,
) => {
  const dataMatriz = getDataMatrizPayments(result, type, isInvoice, status);
  const matriz = getMatrizPayments(
    dataMatriz.payments,
    dataMatriz.cashiers,
    dataMatriz.methodsPayments,
    type,
  );

  let data = getDataCharges(result, type);
  data.forEach((d, i) => {
    const totalsPayments = Decimal.add(
...
const getExtraChargesDetails = (d: VWPaymentExtraCharge) => {
  ...
  extras.forEach((e) => {
    ...
    const index = extras_details.findIndex(
      (ee) => ee.idDetails == detailExtra[0],
    );
    if (index == -1) {
      extras_details.push({
