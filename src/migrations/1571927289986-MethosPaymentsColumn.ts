...
export class MethosPaymentsColumn1571927289986 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<any> {
        await queryRunner.query("ALTER TABLE `facturacion_formas_pago` ADD `showReport` tinyint(1) NOT NULL DEFAULT '1'", undefined);
        ...
        await queryRunner.query("ALTER TABLE `facturacion_bancos` CHANGE `updated_at` `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP", undefined);
    }

    public async down(queryRunner: QueryRunner): Promise<any> {
        await queryRunner.query("ALTER TABLE `facturacion_bancos` CHANGE `updated_at` `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP", undefined);
        ...
        await queryRunner.query("ALTER TABLE `facturacion_formas_pago` DROP COLUMN `showReport`", undefined);
    }
