...
import {
  roundQuantity,
  sumQuantity,
} from '../../../common/point-of-sale/point-of-sale';
...
> {
  constructor(
    ...
    readonly academyRepository: Repository<AcademyCharge>,
  ...
  async fetchFilteredPayments(
    query: QuerySimpleReport,
  ): Promise<AcademyChargePayments[]> {
    const paymentsQueryBuilder = this.repo.createQueryBuilder('payment');
    paymentsQueryBuilder.leftJoinAndSelect(
      'payment.academyPaymentOffice',
      'academyPaymentOffice',
    );
    paymentsQueryBuilder.leftJoinAndSelect(
      'payment.cashierCharge',
      'cashierCharge',
    );
    paymentsQueryBuilder.leftJoinAndSelect(
      'payment.academyCharge',
      'academyCharge',
    );
    paymentsQueryBuilder.leftJoinAndSelect(
    ...
    if (query) {
      ...
      if (query.invoiceStatus) {
        paymentsQueryBuilder.andWhere('payment.stamping = :invoiceStatus', {
          invoiceStatus: query.invoiceStatus,
        });
      }
      if (query.cashier) {
        paymentsQueryBuilder.andWhere('cashierCharge.id = :agentID', {
          agentID: query.cashier,
        });
      }
    }
  ...
  async fetchFilteredSales(query: QuerySimpleReport): Promise<AcademyCharge[]> {
    const salesQueryBuilder = this.academyRepository.createQueryBuilder(
      'charge',
    );
    ...
    salesQueryBuilder.leftJoinAndSelect(
      'charge.schoolStudent',
      'schoolStudent',
    );
    salesQueryBuilder.leftJoinAndSelect(
      'charge.chargesPayments',
      'chargesPayments',
    );
    salesQueryBuilder.leftJoinAndSelect(
    ...
    salesQueryBuilder.leftJoinAndSelect(
      ...
      'academyInscriptionConcept',
    );
    if (query) {
      salesQueryBuilder.where('chargeCampus.id= :officeId', {
        officeId: query.branchOfficeId,
      });
      salesQueryBuilder.andWhere(
        'chargesPayments.paymentStatus= :paymentStatus',
        {
          paymentStatus: query.status,
        },
      );
      ...
      salesQueryBuilder.andWhere(
        'chargesPayments.createdAt BETWEEN :startDate AND :endDate',
        {
          startDate: moment(query.startDate).startOf('day').toDate(),
          endDate: moment(query.endDate).endOf('day').toDate(),
        },
      ...
      if (query.cashier) {
        salesQueryBuilder.andWhere('cashier.id = :agentID', {
          agentID: query.cashier,
        });
      }
    }
  ...
  public async getUserCasher(): Promise<User[]> {
    ...
    const cashiers = cashiersAndSales.filter((cashier) => {
      if (
        (cashier.department !== null && cashier.department.id === 2) ||
        cashier.salePayments.length > 0
      ) {
        return cashier;
      }
    });
  ...
  async simpleReport(
    ...
    const cashiers = cashiersAndSales.filter((cashier) => {
      if (
        (cashier.role.id === 5 && cashier.department.id === 2) ||
        cashier.academyChargesPayments.length > 0
      ) {
        return cashier;
      }
    });
  ...
  async findSaleByPayment(
    query: QueryBillingAcademy,
  ): Promise<{
  ...
  async countTotalPayments(
    ...
    return await this.repo
      ...
      .leftJoinAndSelect(
        'payments.academyPaymentOffice',
        'academyPaymentOffice',
        'academyPaymentOffice.id=:branchOfficeId',
        { branchOfficeId },
      )
      .leftJoinAndSelect('payments.academyCharge', 'academyCharge')
      .leftJoinAndSelect(
        'academyCharge.chargeCycle',
        'chargeCycle',
        'chargeCycle.id=:cycleId',
        { cycleId: ciclyId },
      )
  ...
  public async getGlobalInvoiceFromSales(query: NotInvoicedDto): Promise<any> {
    ...
    data.forEach((value: NotInvoiced) => {
      ...
      if (
        (value.f_status === null || value.f_status === '0') &&
        (value.p_stamping === '0' || value.p_stamping === 0)
      ) {
        unbilledPayments.push(value);

        totalUnbilledPayments = sumQuantity(
    ...
    if (billedPayments.length) {
      const uuids = billedPayments
        .map((value) => value.p_global_uuid)
        .filter((value) => value);
  ...
  public async getGlobalInvoice(
    branchOffice: BranchOffice,
    branchOfficeConfig: BranchOfficeSetting,
  ): Promise<AcademyChargeInvoice> {
    ...
    if (finded?.id) {
      ...
      payload.invoiceBranchOfficeSet = {
        id: branchOfficeConfig.id,
  ...
  public async updateStampingPayments(
    ids: number[],
    uuid: string,
  ): Promise<any> {
    try {
  ...
  public async detailsInvoiceByUuid(params: { uuid: string }) {
  ...
  public async createReceipt(
    ...
    Receip.addReceptor({
      name,
      curp:
        result.payment.stamping == 0 || invoiceFind == undefined
          ? 'XAXX010101000'
          : invoiceFind.rfc,
  ...
  public async cancelPayment(id: number, payload: CancellationDto) {
    ...
    try {
      return await this.connection.transaction(async (manager) => {
        const charge = await manager.findOne(AcademyCharge, {
        ...
        if (!charge) {
          throw new NotFoundException(
            `Venta academia ${payload.saleId} no encontrada`,
          );
        }

        if (charge.status === PaymentStatus.Cancelled) {
          throw new BadRequestException(
            `La venta academia ${payload.saleId} está cancelada`,
          );
        }
        ...
        const newPayment: DeepPartial<AcademyChargePayments> = {
          cashierCharge: { id: payload.cashier },
          academyCharge: { id: payload.saleId },
          academyPaymentOffice: { id: payload.paymentOfficeId },
          academyPaymentOfficeSet: { id: payload.paymentOfficeSetId },
        ...
        const conceptIds = charge.chargesDetails
          .filter(
            (d) =>
              d.academyInscriptionConcept && d.academyInscriptionConcept.id,
          )
      ...
      console.error(
        `Error al agregar pago a venta academia ${payload.saleId}: ${e}`,
      );

      throw new BadRequestException(
        `Error al agregar pago a la venta academia ${payload.saleId}`,
      );
    }
  }
}
