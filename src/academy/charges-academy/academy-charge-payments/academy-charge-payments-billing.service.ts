...
> {
  ...
  async processBilling(query: QueryBillingAcademy): Promise<BillingResponse> {
    return BillingLock.acquire(`academy:${query.chargePaymentId}`, () =>
  ...
  private async processBillingLocked(
    ...
    const existingCheck = await this.checkExistingStamping(
      query.chargePaymentId,
    );
    ...
    if (!invoiceRecord) {
      ...
      invoiceRecord = await this.academyChargeInvoiceService.saveInvoice(
        factura,
      );
    }
    ...
    try {
      ...
      const updatedInvoice = await this.academyChargeInvoiceService.updateInvoice(
        invoiceRecord,
      );
